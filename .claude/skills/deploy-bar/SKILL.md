---
name: deploy-bar
description: Build and deploy lntrn-bar to ~/.local/bin
---

Build and deploy the bar:

1. Run: `cargo build --release -p lntrn-bar`
2. If build succeeds: `cp target/release/lntrn-bar /tmp/lntrn-bar-new && mv -f /tmp/lntrn-bar-new ~/.local/bin/lntrn-bar`
