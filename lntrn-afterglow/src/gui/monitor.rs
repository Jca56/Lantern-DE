...
use super::draw::{
    cpu_temp_color, gpu_temp_color, signed, tile_grid, Dc, CARD_GAP, HEADER_H, PAD, TILE_GAP, TILE_H,
};

/// Draws both cards starting at `y`; CPU on top, GPU below (user preference).
pub fn draw(dc: &mut Dc, info: &Info, snap: &Snapshot, tuning: Option<&CpuTuning>, x: f32, mut y: f32, cw: f32) {
...
fn draw_gpu_card(dc: &mut Dc, info: &Info, snap: &Snapshot, x: f32, y: f32, cw: f32) -> f32 {
    ...
    dc.tile(cells[0], "TEMP", &format!("{}°C", snap.temp_c), None, temp_c, Some(snap.temp_c as f32 / 95.0));
    ...
    dc.tile(cells[2], "GPU LOAD", &format!("{}%", snap.util_pct), None, dc.pal.accent, Some(snap.util_pct as f32 / 100.0));
    ...
    dc.tile(cells[6], "FANS", &fan_txt, Some(&format!("{} fans", snap.fans_pct.len())), dc.pal.info, None);

    y + card_h
}

/// Returns the y just past the card.
fn draw_cpu_card(dc: &mut Dc, info: &Info, snap: &Snapshot, tuning: Option<&CpuTuning>, x: f32, y: f32, cw: f32) -> f32 {
    let s = dc.s;
    let grid_h = 96.0 * s;
    ...
    dc.label(&info.cpu_model, x + PAD * s, y + 14.0 * s, 24.0, dc.pal.text);
    let p = snap.cores.iter().filter(|c| c.is_pcore).count();
    let sub = format!("{} threads · {p} P · {} E", snap.cores.len(), snap.cores.len() - p);
    dc.label(&sub, x + PAD * s, y + 40.0 * s, 14.0, dc.pal.text_secondary);

    let region = Rect::new(x + PAD * s, y + HEADER_H * s, cw - PAD * 2.0 * s, TILE_H * s);
    ...
    dc.tile(cells[0], "PKG TEMP", &temp_val, None, temp_col, snap.cpu_temp_c.map(|c| c / 100.0));
    dc.tile(cells[1], "CPU LOAD", &format!("{:.0}%", snap.cpu_util_pct), None, dc.pal.accent, Some(snap.cpu_util_pct / 100.0));

    let pwr = match snap.pkg_w {
        Some(p) => format!("{p:.0} W"),
        None => "—".to_string(),
    };
    let pl_sub = if info.rapl { format!("PL1 {} W", info.pl1_w) } else { "RAPL n/a".to_string() };
    let pwr_frac = match (snap.pkg_w, info.pl1_w) {
        (Some(p), pl) if pl > 0 => Some(p / pl as f32),
        _ => None,
    };
    dc.tile(cells[2], "PKG POWER", &pwr, Some(&pl_sub), dc.pal.accent, pwr_frac);
...
fn draw_core_grid(dc: &mut Dc, snap: &Snapshot, x: f32, y: f32, w: f32, h: f32) {
    ...
    for (i, core) in snap.cores.iter().enumerate() {
        ...
        let base = if core.is_pcore { dc.pal.accent } else { dc.pal.info };
        if fill_h > 0.0 {
            dc.p.rect_filled(
...
pub fn draw_offline(dc: &mut Dc, status: &str, x: f32, y: f32, cw: f32, hf: f32) {
    ...
    dc.p.rect_filled(card, 14.0 * s, dc.pal.surface);
    let cy = y + 60.0 * s;
    dc.label("afterglowd not reachable", x + PAD * s, cy, 24.0, dc.pal.danger);
    dc.label(status, x + PAD * s, cy + 38.0 * s, 16.0, dc.pal.text_secondary);
    dc.label("Start the daemon (boot service):", x + PAD * s, cy + 76.0 * s, 16.0, dc.pal.text);
    dc.label("sudo rc-service lntrn-afterglowd start", x + PAD * s, cy + 104.0 * s, 16.0, dc.pal.accent);
}
