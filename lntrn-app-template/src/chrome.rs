...
pub fn draw_background(p: &mut Painter, wf: f32, hf: f32, r: f32) {
    ...
    p.rect_gradient_radial(
        Rect::new(-wf * 0.35, hf * 0.5, wf * 0.8, hf * 0.8),
        0.0,
        GLOW_PINK,
        Color::TRANSPARENT,
    );
    p.rect_gradient_radial(
        Rect::new(wf * 0.5, -hf * 0.25, wf * 0.8, hf * 0.7),
        0.0,
        GLOW_CYAN,
        Color::TRANSPARENT,
    );
...
pub fn draw_controls(p: &mut Painter, cx: f32, cy: f32, s: f32, wf: f32, title_h: f32) {
    ...
    if hov {
        p.circle_filled(close_cx, btn_y, btn_r, CLOSE_HOVER);
    }
    ...
    if hov {
        p.circle_filled(max_cx, btn_y, btn_r, CONTROL_HOVER);
    }
    ...
    if hov {
        p.circle_filled(min_cx, btn_y, btn_r, CONTROL_HOVER);
    }
    let ic = if hov { TEXT_PRIMARY } else { CONTROL_ICON };
