...
use lntrn_ui::gpu::{
    ContextMenu, FontSize, FoxPalette, InteractionContext, MenuEvent,
...
/// Draw the entire Files panel (nav, tabs, sidebar, content, status, menus)
/// and perform the layered GPU render/submit. Returns context and view menu events.
...
pub fn draw_files_panel(
    ...
    let Gpu { ctx, painter, text, mono_text: _, tex_pass } = gpu;
    ...
    let total_content_h = match view_mode {
        ...
        ViewMode::List => list_content_height(entries.len(), s) + 32.0 * s,
    ...
    // ── Nav bar ──────────────────────────────────────────────────────────
    let nav_rect = nav_bar_rect(wf, s).translate(ox, 0.0);
    let vt_rect = view_toggle_rect(s).translate(ox, 0.0);
    let vt_state = input.add_zone(ZONE_NAV_VIEW_TOGGLE, vt_rect);
    let back_rect = back_button_rect(s).translate(ox, 0.0);
    let back_state = input.add_zone(ZONE_NAV_BACK, back_rect);
    let fwd_rect = forward_button_rect(s).translate(ox, 0.0);
    let fwd_state = input.add_zone(ZONE_NAV_FORWARD, fwd_rect);
    let up_rect = up_button_rect(s).translate(ox, 0.0);
    let up_state = input.add_zone(ZONE_NAV_UP, up_rect);
    let p_rect = path_rect(wf, s).translate(ox, 0.0);
    let path_zone = input.add_zone(ZONE_PATH_INPUT, p_rect);
    let srch_rect = search_button_rect(wf, s).translate(ox, 0.0);
    ...
    draw_gradient_h(painter, pal, ox, nav_rect.y + nav_rect.h, wf, s);

    // ── Tab bar ─────────────────────────────────────────────────────
    let tab_rect = tab_bar_rect(wf, s).translate(ox, 0.0);
    ...
    // ── Sidebar ───────────────────────────────────────────────────────
    let sidebar = sidebar_rect(hf, s).translate(ox, 0.0);
    let places = app.sidebar_places();
    let mut sidebar_hovered = Vec::with_capacity(places.len());
    for i in 0..places.len() {
        let item_rect = sidebar_item_rect(i, s).translate(ox, 0.0);
    ...
    for i in 0..app.drives.len() {
        let item_rect = drive_item_rect(i, num_places, s).translate(ox, 0.0);
        let zone_id = ZONE_DRIVE_ITEM_BASE + i as u32;
        let state = input.add_zone(zone_id, item_rect);
        drive_hovered.push(state.is_hovered());
    }
    let dragging = app.drag_item.is_some();
    draw_sidebar(painter, text, pal, app, sidebar, &sidebar_hovered, &drive_hovered, dragging, (w, h), s, ox);

    // ── Content area ───────────────────────────────────────────────────
    let content = content.translate(ox, 0.0);
    let base_y = base_y; // unchanged (vertical only)
    ...
    match view_mode {
        ViewMode::Grid => {
            ...
            draw_content_grid(
                ...
                &scroll_area, &item_hovered, &has_icon, app.drag_item, app.renaming, (w, h), s, zoom, ox,
            );
        ...
        ViewMode::List => {
            ...
            draw_content_list(
                ...
                &scroll_area, &item_hovered, &has_icon, app.drag_item, app.renaming, (w, h), s, ox,
            );
        }
        ViewMode::Tree => {
            let row_h = tree_row_h(s);
            ...
            draw_content_tree(
                ...
                &scroll_area, &item_hovered, &has_icon, (w, h), s, ox,
            );
        }
    }
    ...
    if scroll_area.is_scrollable() {
        ...
        draw_scrollbar(painter, &scrollbar, sb_state, pal, ox);
    }

    // ── Status bar ──────────────────────────────────────────────────
    {
        let status = status_rect(wf, hf, s).translate(ox, 0.0);
        draw_status_bar(painter, text, pal, status, &app.entries, file_info, (w, h), s, ox);
    }

    // ── Rubber band selection overlay ─────────────────────────────────
    if let (Some(start), Some(end)) = (app.rubber_band_start, app.rubber_band_end) {
        draw_rubber_band(painter, pal, start, end, content, ox);
    }
