...
pub fn run() -> Result<()> {
    ...
    let mut panel = crate::panel::PanelState::new();
    let mut active_tab: usize = 0;
    let tab_names = ["Lighting"];
    ...
    while state.running {
        ...
        // Slider drag
        crate::panel::handle_drag(cx, cy, s, title_h + 44.0 * s, hf, &mut panel);

        // Left release
        if state.left_released {
            state.left_released = false;
            crate::panel::handle_release(&mut panel);
            if let Some(pid) = pointer_on_popup {
                if let Some(backend) = &mut state.popup_backend {
                    if let Some(ctx) = backend.popup_render(pid) {
        ...
        crate::chrome::draw_title(&mut text, "Lantern Gear", s, wf, title_h, sw, sh);
        ...
        let tab_border = Color::rgba(0.30, 0.20, 0.50, 0.12);
        {
            ...
            for i in 0..tab_names.len() {
                ...
                if is_active {
                    ...
                    painter.rect_filled(
                        Rect::new(tx, tab_bar_y, tw, tab_h),
                        8.0 * s,
            ...
            // Bottom line
            painter.rect_filled(
                Rect::new(tab_pad, tab_bar_y + tab_h, wf - tab_pad * 2.0, 1.0 * s),
                0.0,
                tab_border,
            );
        }
