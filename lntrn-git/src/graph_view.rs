...
impl GraphView {
    ...
    fn compute_layout(commits: &[git::GraphCommit]) -> Vec<LayoutNode> {
        ...
        let mut hash_to_row: std::collections::HashMap<&str, usize> =
        ...
        for (_row, commit) in commits.iter().enumerate() {
            ...
            let my_lane = lanes
                .iter()
                .position(|slot| slot.as_ref().map_or(false, |h| h == &commit.hash));
            ...
            for (pi, parent_hash) in commit.parents.iter().enumerate() {
                if let Some(&target_row) = hash_to_row.get(parent_hash.as_str()) {
                    if pi == 0 {
                        ...
                        let existing = lanes
                            .iter()
                            .position(|slot| slot.as_ref().map_or(false, |h| h == parent_hash));
                        let parent_lane = if let Some(l) = existing {
                            l
                        } else {
                            let free = lanes.iter().position(|s| s.is_none());
                            let l = if let Some(f) = free {
                                f
                            } else {
                                lanes.push(None);
                                lanes.len() - 1
    ...
    pub fn draw(
        ...
        for (row, node) in self.layout.iter().enumerate() {
            ...
            for &(from_lane, to_lane, target_row) in &node.edges {
                ...
                if (y1 < cy && y2 < cy) || (y1 > cy + ch && y2 > cy + ch) {
                    continue;
                }
        ...
        for (row, (commit, node)) in self.commits.iter().zip(self.layout.iter()).enumerate() {
            ...
            if !commit.decorations.is_empty() {
                let mut dx =
                    subject_x + commit.subject.len().min(50) as f32 * body_font * 0.5 + 8.0 * s;
                for dec in &commit.decorations {
                    ...
                    let badge_color = if is_head {
                        palette.accent
                    } else {
                        palette.muted
                    };
                    painter.rect_filled(
                        Rect::new(dx, by, bw, bh),
                        3.0 * s,
                        badge_color.with_alpha(0.2),
                    );
                    text.queue(
                        &label,
                        badge_font,
                        dx + 5.0 * s,
                        by + (bh - badge_font) * 0.5,
                        badge_color,
                        bw,
                        sw,
                        sh,
