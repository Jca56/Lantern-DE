...
impl MergeModal {
    ...
    pub fn on_click(&mut self, ix: &InteractionContext, px: f32, py: f32) -> MergeAction {
        ...
        // Dropdown toggles
        if zone == ZONE_SOURCE_BTN {
            self.source_open = !self.source_open;
            self.target_open = false;
            return MergeAction::None;
        }
        if zone == ZONE_TARGET_BTN {
    ...
    pub fn draw(
        ...
        let dropdown_items = if self.source_open || self.target_open {
            self.branches.len() as f32 * row_h + 8.0 * s
        } else {
            0.0
        };
        let panel_h = pad * 2.0
        ...
        let btn_gap = 12.0 * s;
        let can_merge = self.source_idx.is_some()
            && self.target_idx.is_some()
            && self.source_idx != self.target_idx;

        let cancel_rect = Rect::new(
            px + panel_w - pad - cancel_w - btn_gap - merge_w,
            y,
            cancel_w,
            btn_h,
        );
        let cancel_state = ix.add_zone(ZONE_CANCEL_BTN, cancel_rect);
        let cancel_c = if cancel_state.is_hovered() {
        ...
        text.queue(
            "Cancel",
            small,
            cancel_rect.x + (cancel_w - ctw) * 0.5,
            cty,
            palette.text,
            cancel_w,
            sw,
        ...
        let merge_c = if !can_merge {
            palette.muted.with_alpha(0.3)
        } else if merge_state.is_hovered() {
            palette.accent
        } else {
    ...
    fn draw_dropdown(
        ...
        let bg = if open || btn_state.is_hovered() {
            palette.surface_2
        } else {
            palette.surface
        };
