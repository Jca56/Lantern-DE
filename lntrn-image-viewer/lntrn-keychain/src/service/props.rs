...
use super::paths::{self, ObjectKind, IFACE_COLLECTION, IFACE_ITEM, IFACE_SERVICE};
use super::service_iface;
use super::state::ServiceState;
use super::wire::{encode_variant_dict_ss, encode_variant_object_paths, encode_variant_u64};
use crate::storage;
...
fn handle_get(conn: &mut Connection, msg: &Message, state: &ServiceState, iface: &str, prop: &str) {
...
fn handle_get_all(conn: &mut Connection, msg: &Message, state: &ServiceState, iface: &str) {
    let mut body = Vec::new();
...
fn encode_property(
    ...
    match (paths::classify(path), iface, prop) {
        (ObjectKind::Service, IFACE_SERVICE, "Collections") => {
            let mut paths_v: Vec<String> = state
                .collections
                .keys()
                .map(|k| paths::collection_path(k))
                .collect();
            paths_v.sort();
        ...
        (ObjectKind::Collection(id), IFACE_COLLECTION, "Items") => {
            let coll = match state.collections.get(id) {
                Some(c) => c,
                None => return false,
            };
            let mut item_paths: Vec<String> =
                coll.items.keys().map(|k| paths::item_path(id, k)).collect();
            item_paths.sort();
        ...
        (ObjectKind::Item(c, i), IFACE_ITEM, "Locked") => {
            let coll = match state.collections.get(c) {
                Some(c) => c,
                None => return false,
            };
...
fn encode_all_properties(out: &mut Vec<u8>, state: &ServiceState, path: &str, iface: &str) {
    align_to(out, 4);
    let len_pos = out.len();
    encode_u32(out, 0);
    align_to(out, 8);
...
fn apply_set(state: &mut ServiceState, path: &str, iface: &str, prop: &str, value: Value) -> bool {
    match (paths::classify(path), iface, prop) {
        (ObjectKind::Collection(id), IFACE_COLLECTION, "Label") => {
            ...
            let coll = match state.collections.get_mut(id) {
                Some(c) => c,
                None => return false,
            };
            coll.label = s;
        ...
        (ObjectKind::Item(c, i), IFACE_ITEM, "Label") => mutate_item(state, c, i, |it| {
            if let Some(s) = value.as_str() {
                it.label = s.to_string();
            }
        }),
        (ObjectKind::Item(c, i), IFACE_ITEM, "Type") => mutate_item(state, c, i, |it| {
            if let Some(s) = value.as_str() {
                it.content_type = s.to_string();
            }
        }),
        (ObjectKind::Item(c, i), IFACE_ITEM, "Attributes") => {
