...
pub fn dispatch(
    ...
    if msg.interface != IFACE_ITEM { return false; }
    match msg.member.as_str() {
        "Delete" => { delete(conn, msg, state, coll_id, item_id); true }
        "GetSecret" => { get_secret(conn, msg, state, coll_id, item_id); true }
        "SetSecret" => { set_secret(conn, msg, state, coll_id, item_id); true }
        _ => false,
    }
}
...
fn get_secret(
    ...
    let resolved_session = session_id.map(paths::session_path)
        .unwrap_or_else(|| session_path.clone());

    let mut body = Vec::new();
    encode_secret_struct(&mut body, &resolved_session, &params, &value, &it.content_type);
...
fn set_secret(
    ...
    let coll = match state.collections.get_mut(coll_id) {
        ...
        None => {
            conn.send_error(msg.serial, &msg.sender,
                "org.freedesktop.Secret.Error.NoSuchObject", "no such collection");
            return;
        }
    };
    let it = match coll.items.get_mut(item_id) {
        ...
        None => {
            conn.send_error(msg.serial, &msg.sender,
                "org.freedesktop.Secret.Error.NoSuchObject", "no such item");
            return;
        }
    };
    it.secret = plaintext;
    if !content_type.is_empty() { it.content_type = content_type; }
