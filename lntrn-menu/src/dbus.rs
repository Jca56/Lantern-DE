...
fn encode_signature(buf: &mut Vec<u8>, s: &str) {
        ...
        Some(match c {
            ...
            'v' => {
                ...
                let vsig = String::from_utf8_lossy(
                    &self.data[self.pos..self.pos + vsig_len]
                ).to_string();
                self.pos += vsig_len + 1; // skip null
            ...
            '(' => {
                // Read struct fields based on remaining sig chars
                self.align(8);
                let inner = &sig[1..sig.len()-1]; // strip parens
                let mut fields = Vec::new();
                let mut i = 0;
                while i < inner.len() {
                    let field_sig = &inner[i..i+1];
