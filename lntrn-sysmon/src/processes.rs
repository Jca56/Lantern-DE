...
impl ProcessSampler {
    ...
    pub fn sample(&mut self, sort: Sort) -> Vec<ProcessEntry> {
        ...
        for entry in entries.flatten() {
            ...
            let Some((comm, post)) = parse_stat_comm(&stat) else {
                continue;
            };
            if comm.is_empty() {
                continue;
...
pub fn draw(
    ...
    p.rect_filled(
        Rect::new(pad, hy, wf - pad * 2.0, header_h),
        8.0 * s,
        HEADER_BG,
    );
    ...
    for (i, proc) in procs.iter().enumerate() {
        ...
        if is_selected {
            p.rect_filled(
                Rect::new(pad, y, wf - pad * 2.0, row_h),
                4.0 * s,
                ROW_SELECTED,
            );
        } else if hov {
