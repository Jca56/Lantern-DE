...
/// Draw the window background. Two layers:
///   * Solid bg color — from `[appearance].background_color` if set, else
///     the FOX/LN variant default. Tinted by
///     `[windows].background_opacity`.
///   * Optional gradient overlay — `[appearance].window_gradient_stops`
///     painted on top with per-stop alphas, so dialing a stop down lets the
///     solid bg color show through at that band instead of becoming
///     window-transparent.
pub fn draw_background(p: &mut Painter, mode: WindowMode, wf: f32, hf: f32, r: f32) {
    let opacity = lntrn_theme::background_opacity();
    let rect = Rect::new(0.0, 0.0, wf, hf);
...
pub fn draw_controls(
    ...
    if hov {
        p.circle_filled(close_cx, btn_y, btn_r, pal.close_hover);
    }
    ...
    if hov {
        p.circle_filled(max_cx, btn_y, btn_r, pal.control_hover);
    }
    let ic = if hov {
        pal.text_primary
    } else {
    ...
    if hov {
        p.circle_filled(min_cx, btn_y, btn_r, pal.control_hover);
    }
    let ic = if hov {
        pal.text_primary
    } else {
