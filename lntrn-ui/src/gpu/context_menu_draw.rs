...
use super::context_menu::{
    items_height_slice, ContextMenuStyle, MenuEvent, MenuItem, ACCENT_BAR_WIDTH,
    COLOR_SWATCH_HEIGHT, CONTEXT_MENU_ZONE_BASE, HEADER_HEIGHT, PROGRESS_ITEM_HEIGHT,
    SEPARATOR_HEIGHT, SLIDER_ITEM_HEIGHT, SLIDER_TRACK_H, TAB_DOTS_OVERHANG,
...
pub(super) fn draw_panel(
    ...
    if !style.no_shadow {
        let shadow_layers: [(f32, f32); 3] = [(12.0, 0.12), (5.0, 0.18), (2.0, 0.25)];
    ...
    for item in items.iter_mut() {
        match item {
            ...
            MenuItem::Toggle {
                ...
                let zone_id = zone_base + *id;
                let state = zone_state(interaction, zone_id, item_rect);
                if state.is_hovered() && *enabled {
                    non_submenu_hovered = true;
                }

                draw_hover_bg(
                    state.is_hovered() && *enabled,
                    item_rect,
                    cr,
                    s,
                    style,
            ...
            MenuItem::Checkbox { id, label, checked } => {
                ...
                if state == InteractionState::Pressed && !pressed_zones.contains(&zone_id) {
                    pressed_zones.push(zone_id);
                    *checked = !*checked;
                    event = Some(MenuEvent::CheckboxToggled {
                        id: *id,
                        checked: *checked,
                    });
                }
            ...
            MenuItem::Radio {
                    ...
                    && !pressed_zones.contains(&zone_id)
                {
                    pressed_zones.push(zone_id);
                    *selected = true;
                    event = Some(MenuEvent::RadioSelected {
                        id: *id,
                        group: *group,
                    });
                }
            ...
            MenuItem::Progress {
                ...
                let bar_rect = Rect::new(content_x, bar_y, content_w - pad, 14.0 * s);
            ...
            MenuItem::Separator => {
                ...
                painter.rect_filled(
                    Rect::new(sep_x, sep_y, sep_w, 1.0 * s),
                    0.0,
                    style.separator,
                );
                cy += sep_h;
            ...
            MenuItem::ColoredSeparator(color) => {
                ...
                let sep_x = content_x;
                let sep_w = content_w - pad;
                painter.rect_filled(Rect::new(sep_x, sep_y, sep_w, 2.0 * s), 1.0 * s, *color);
                cy += sep_h;
            }
            MenuItem::Slider { id, label, value } => {
                ...
                let thumb_r = if zone_state.is_active() {
                    9.0 * s
                ...
                if zone_state.is_active() {
                    if let Some(frac) = interaction.drag_fraction_x(&track) {
                        *value = frac;
                        event = Some(MenuEvent::SliderChanged {
                            id: *id,
                            value: frac,
                        });
                    }
...
fn zone_state(interaction: &mut InteractionContext, zone_id: u32, rect: Rect) -> InteractionState {
