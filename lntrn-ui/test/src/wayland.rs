...
pub(crate) struct State {
    ...
    // Popup
    pub(crate) popup_backend: Option<crate::popup_backend::WaylandPopupBackend>,
...
pub fn run() -> Result<()> {
    ...
    // Initialize popup backend (clone xdg_surface to avoid borrow conflict)
    {
        ...
        state.popup_backend = Some(crate::popup_backend::WaylandPopupBackend::new(
    ...
    while state.running {
        ...
        // Route cursor to popup InteractionContext
            ...
            if let Some(backend) = &mut state.popup_backend {
                if let Some(ctx) = backend.popup_render(pid) {
                    if state.pointer_in_surface {
                        ctx.interaction.on_cursor_moved(cx, cy);
                    } else {
                        ctx.interaction.on_cursor_left();
                    }
                }
            }
        }
        ...
        if let Ok(mut frame) = gpu.begin_frame("ui-test") {
            ...
            painter.render_pass(&gpu, frame.encoder_mut(), &view, lntrn_render::Color::TRANSPARENT);
