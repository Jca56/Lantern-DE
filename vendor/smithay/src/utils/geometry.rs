...
macro_rules! unsigned_coordinate_impl {
    ...
    ($ty:ty) => {
        impl Coordinate for $ty {
            ...
            #[inline]
            fn downscale(self, scale: Self) -> Self {
                // Lantern patch: guard against a zero integer scale (see the
                // signed impl below for the full rationale) to avoid a
                // divide-by-zero panic on degenerate X11 surfaces.
...
macro_rules! signed_coordinate_impl {
    ...
    ($ty:ty) => {
        impl Coordinate for $ty {
            ...
            #[inline]
            fn downscale(self, scale: Self) -> Self {
                // Lantern patch: a zero integer scale reaches this path when an
                // X11 client (Steam's CEF/override-redirect surfaces) commits a
                // degenerate viewport while the XWayland client_scale is < 1.0.
                // Upstream Smithay 0.7 divides unconditionally and panics with
                // "attempt to divide by zero". Treat scale 0 as identity (no
                // scaling) so the compositor survives the bad surface instead.
                if scale == 0 { return self; }
                self / scale
            }
