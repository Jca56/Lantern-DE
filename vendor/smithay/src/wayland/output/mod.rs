...
impl Output {
    ...
    /// Lantern addition: re-send this output's current mode/scale/geometry
    /// (wl_output) and logical size/position (xdg_output) to a SINGLE client's
    /// bound instances only.
    ///
    /// A global [`Output::change_current_state`] re-advertises to EVERY client
    /// (it always sends `wl_output.done()` to all instances), which disturbs
    /// unrelated layer-shell daemons. This method targets just `client` — used
    /// to push a `client_scale` change to XWayland after it has already bound
    /// the outputs at the default scale, so games see native resolution without
    /// touching any other client.
    pub fn resend_state_to_client(&self, client: &Client) {
        let inner = self.inner.0.lock().unwrap();
        let target = client.id();

        // xdg_output carries the logical size XWayland turns into RandR modes.
        if let Some(xdg_output) = inner.xdg_output.as_ref() {
            xdg_output.resend_to_client(&target);
        }
        ...
        for output in &inner.instances {
            ...
            if output.client().map(|c| c.id()).as_ref() != Some(&target) {
                continue;
            }
            let data = output.data::<OutputUserData>().unwrap();
            let client_scale = data.client_scale.load(Ordering::Acquire);
            data.last_client_scale.swap(client_scale, Ordering::AcqRel);

            if let Some(mode) = inner.current_mode {
                let mut flags = WMode::Current;
