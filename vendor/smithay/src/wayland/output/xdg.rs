...
use wayland_server::{backend::ClientId, protocol::wl_output::WlOutput, Resource, Weak};
...
impl XdgOutput {
    ...
    /// Lantern addition: force-resend the current logical size + position to a
    /// SINGLE client's xdg_output instances only. Used to push a client_scale
    /// change to XWayland after it has already bound the output (at the default
    /// scale) without re-advertising to every other client. See
    /// `super::Output::resend_state_to_client`.
    pub(super) fn resend_to_client(&self, client: &ClientId) {
        let output = self.inner.lock().unwrap();
        for instance in output.instances.iter() {
            let Ok(instance) = instance.upgrade() else {
                continue;
            };
            if instance.client().map(|c| c.id()).as_ref() != Some(client) {
                continue;
            }
            let data = instance.data::<XdgOutputUserData>().unwrap();
            let client_scale = data.client_scale.load(Ordering::Acquire);
